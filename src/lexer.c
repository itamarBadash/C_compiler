...
static token lexer_collect_string(lexer *lex, token_type type) {
  ...
  if (lex->current_char == '\0') {
    lexer_error(lex, start_line, start_column, "unterminated string literal");
...
static token lexer_collect_char_literal(lexer *lex, token_type type) {
  int start_line = lex->line;
  int start_column = lex->column;
  ...
  if (lex->current_char == '\\') {
    lexer_advance(lex);
    if (lex->current_char == '\0') {
      lexer_error(lex, start_line, start_column, "unterminated character constant");
      return lexer_make_token(lex, TOKEN_UNKNOWN, NULL);
    }
    if (lex->current_char >= '0' && lex->current_char <= '7') {
      int digits = 0;
      while (digits < 3 && lex->current_char >= '0' && lex->current_char <= '7') {
        lexer_advance(lex);
        digits++;
      }
    } else if (lex->current_char == 'x') {
      lexer_advance(lex);
      while (isxdigit((unsigned char)lex->current_char)) {
        lexer_advance(lex);
      }
    } else {
      lexer_advance(lex);
  ...
  if (lex->current_char != '\'') {
    if (lex->current_char == '\0') {
      ...
      lexer_error(lex, start_line, start_column, "invalid character constant");
    }
    return lexer_make_token(lex, TOKEN_UNKNOWN, NULL);
  }
...
token lexer_next_token(lexer *lex) {
  if (!lex || !lex->source) {
    token tok = {TOKEN_UNKNOWN, NULL, 0, 0, 0};
    return tok;
  }
  ...
  case '^':
    ...
    break;
  case '\'':
    tok = lexer_collect_char_literal(lex, TOKEN_CHAR_LITERAL);
    break;
  case '"':
    tok = lexer_collect_string(lex, TOKEN_STRING);
    break;

  default:
    if (lex->current_char == 'L' && (lexer_peek(lex) == '"' || lexer_peek(lex) == '\'')) {
      lexer_advance(lex);
      tok = (lex->current_char == '"') ? lexer_collect_string(lex, TOKEN_WIDE_STRING)
                                       : lexer_collect_char_literal(lex, TOKEN_WIDE_CHAR);
