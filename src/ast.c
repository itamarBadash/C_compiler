...
void free_ast(ast_node *node) {
  ...
  case AST_NODE_TYPE_CHAR_LITERAL:
    if (node->literal.bytes != NULL) {
      free(node->literal.bytes);
    }
    break;
  ...
  case AST_NODE_TYPE_STRUCT_DEF:
    if (node->struct_def.tag_name)
      free(node->struct_def.tag_name);
  ...
  case AST_NODE_TYPE_ENUM_DEF:
    if (node->enum_def.tag_name)
      free(node->enum_def.tag_name);
  ...
  case AST_NODE_TYPE_GOTO:
    if (node->goto_stmt.label_name)
      free(node->goto_stmt.label_name);
    break;
